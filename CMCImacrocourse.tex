...
syntax for commands but doing this will be very very helpful afterward. You
...
(or function), since \ilcom{a} is known to be a number as it was defined so in one of the lines above. Then ImageJ goes on interpreting rightward thinking that this is math. Then finding a "+" which surprizingly is a character
...
\item After this \ilcom{for(\ldots;\ldots;\ldots)} command, there is a brace (\{) at the end of line 4 and the second one in the line 6. These curly braces tell ImageJ to loop commands in between so the command in line 5 will be iterated according to the parameters defined in the parenthesis of \ilcom{for}. Between braces, you could add more lines of commands as many as you want.  
\end{itemize}
...
Then in line 6 interpreter sees the boundary \ilcom{\}} and goes back to line 4 and adds 1 to i (because of \ilcom{i+=1}). i = 1 then, so \ilcom{i<5} is true. The interpreter proceeds to line 5 and executes the command and prints out "1\ensuremath\colon whatever".  Such looping will continue until i = 5, since only by then \ilcom{i<5} is no longer true so interpreter exits from the for-loop. \\
