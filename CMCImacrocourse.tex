...
Above example might be already clear, but we examine the detail of the
content between parenthesis after ``if''. Write the following code in
your script editor and run it.
\lstinputlisting[morekeywords={*, ==}]{code/code12_1.ijm}
The output in the log window should be \textit{1} indicating that \ilcom{(5 ==
5)} returned \textit{1}. Next, modify the code like below and run it.
\lstinputlisting[morekeywords={*, ==}]{code/code12_2.ijm}
The output becomes \text{0}. The second line \ilcom{(5 == 4)} is 0. 
What double equal sign is doing in these
...
0 actually are representing \textbf{true} (= 1) or \textbf{false} (= 0). 
